...
/**
 * 组件基类,实现了 *property*, *event* and *behavior* 特性.
 *
 * 组件提供了 *event* and *behavior* 特性, *property* 特性在他的父类中实现 [[Object]].
 *
 ...
 * - 对象名和方法名数组形式指定的对象方法，如: `[$object, 'handleAdd']`
 * - 类名和方法名数组形式指定的静态类方法，如: `['Page', 'handleAdd']`
 * - 字符串形式指定的 PHP 全局函数，如: `'handleAdd'`
 *
 * 事件处理器的格式是:
 *
 * ~~~
 ...
 * ~~~
 *
 * where `as tree` stands for attaching a behavior named `tree`, and the array will be passed to [[\Leaps::createObject()]]
...
class Service extends Object
{
	...
	public function __set($name, $value)
	{
		...
		if (method_exists ( $this, $setter )) {
			...
			$this->attachBehavior ( $name, $value instanceof Behavior ? $value : Leaps::createObject ( $value ) );
			return;
		} else {
			// 行为属性
			$this->ensureBehaviors ();
	...
	/**
	 * 触发一个事件
	 * This method represents the happening of an event.
	 * It invokes
	 ...
	 *        - an object configuration array that will be passed to [[Leaps::createObject()]] to create the behavior object.
	 *       
	...
	private function attachBehaviorInternal($name, $behavior)
	{
		if (! ($behavior instanceof Behavior)) {
			$behavior = Leaps::createObject ( $behavior );
		}
		if (is_int ( $name )) {
