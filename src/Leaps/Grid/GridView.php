...
use Leaps\Widgets\BaseListView;
...
class GridView extends BaseListView
{
     ...
     * Defaults to 'Leaps\Grid\DataColumn'.
     */
     ...
     * the [[\Leaps\I18n\Formatter::nullDisplay|nullDisplay]] property of the [[formatter]] will
     * be used to indicate an empty data value.
     */
    public $emptyCell = '&nbsp;';
    /**
     * @var \Leaps\Base\Model the model that keeps the user-entered filter data. When this property is set,
    ...
    public function init()
    {
        ...
        if ($this->formatter == null) {
            $this->formatter = Leaps::$app->getFormatter();
        } elseif (is_array($this->formatter)) {
            $this->formatter = Leaps::createObject($this->formatter);
        }
    ...
    public function run()
    {
        ...
        $view->registerJs("jQuery('#$id').leapsGridView($options);");
        parent::run();
    }
    ...
    protected function initColumns()
    {
        ...
        foreach ($this->columns as $i => $column) {
            if (is_string($column)) {
                ...
                $column = Leaps::createObject(array_merge([
