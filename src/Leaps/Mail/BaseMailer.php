...
abstract class BaseMailer extends Service implements MailerInterface, ViewContextInterface
{
    ...
    /**
     * @var \Leaps\Base\View|array view instance or its array configuration.
     */
    private $_view = [];
    /**
    ...
    public function compose($view = null, array $params = [])
    {
        ...
        if (isset($text)) {
            ...
            $text = html_entity_decode(strip_tags($html), ENT_QUOTES | ENT_HTML5, Leaps::$app ? Leaps::$app->charset : 'UTF-8');
            // improve whitespace
    ...
    public function send($message)
    {
        ...
        Leaps::info('Sending email "' . $message->getSubject() . '" to "' . $address . '"', __METHOD__);
    ...
    {
        $path = Leaps::getAlias($this->fileTransportPath);
        if (!is_dir(($path))) {
