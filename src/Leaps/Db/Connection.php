 ...
 *
 * ~~~
 * $connection = new \Leaps\Db\Connection([
 *     'dsn' => $dsn,
 *     'username' => $username,
 *     'password' => $password,
 * ]);
...
class Connection extends Service
{
     ...
     * Use 0 to indicate that the cached data will never expire.
     * @param \Leaps\Cache\Dependency $dependency the cache dependency associated with the cached query results.
     ...
     * @param \Leaps\Cache\Dependency $dependency the preferred caching dependency. If null, it will be ignored.
    ...
    public function getQueryCacheInfo($duration, $dependency)
    {
        ...
        if ($duration === 0 || $duration > 0) {
            if (is_string($this->queryCache) && Leaps::$app) {
                $cache = Leaps::$app->get($this->queryCache, false);
            } else {
    ...
    protected function openFromPool(array $pool, array $sharedConfig)
    {
        ...
        foreach ($pool as $config) {
            ...
            try {
                ...
                Leaps::warning("Connection ({$config['dsn']}) failed: " . $e->getMessage(), __METHOD__);
                if ($cache instanceof Cache) {
