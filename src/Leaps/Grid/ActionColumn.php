    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Leaps::t('leaps', 'View'),
                    'aria-label' => Leaps::t('leaps', 'View'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
        ...
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Leaps::t('leaps', 'Update'),
                    'aria-label' => Leaps::t('leaps', 'Update'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Leaps::t('leaps', 'Delete'),
                    'aria-label' => Leaps::t('leaps', 'Delete'),
                    'data-confirm' => Leaps::t('leaps', 'Are you sure you want to delete this item?'),
                    'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);
