 ...
 *
 * User is configured as an application component in [[\Leaps\Web\Application]] by default.
 * You can access that instance via `Leaps::$app->user`.
 *
...
class User extends Component
{
    ...
    public function login(IdentityInterface $identity, $duration = 0)
    {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            $id = $identity->getId();
            $ip = Leaps::$app->getRequest()->getUserIP();
            if ($this->enableSession) {
                $log = "User '$id' logged in from $ip with duration $duration.";
            } else {
                $log = "User '$id' logged in from $ip. Session not enabled.";
            }
            Leaps::info($log, __METHOD__);
    ...
    {
        $value = Leaps::$app->getRequest()->getCookies()->getValue($this->identityCookie['name']);
        if ($value === null) {
            return;
        }
        ...
        if ($identity->validateAuthKey($authKey)) {
            if ($this->beforeLogin($identity, true, $duration)) {
                ...
                $ip = Leaps::$app->getRequest()->getUserIP();
                Leaps::info("User '$id' logged in from $ip via cookie.", __METHOD__);
                $this->afterLogin($identity, true, $duration);
            }
        } else {
            Leaps::warning("Invalid auth key attempted for user '$id': $authKey", __METHOD__);
        }
    }
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Leaps::$app->getRequest()->getUserIP();
            Leaps::info("User '$id' logged out from $ip.", __METHOD__);
            if ($destroySession && $this->enableSession) {
                Leaps::$app->getSession()->destroy();
            }
    ...
    {
        $url = Leaps::$app->getSession()->get($this->returnUrlParam, $defaultUrl);
        if (is_array($url)) {
            if (isset($url[0])) {
                return Leaps::$app->getUrlManager()->createUrl($url);
            } else {
                $url = null;
            }
        }

        return $url === null ? Leaps::$app->getHomeUrl() : $url;
    }
    ...
    {
        Leaps::$app->getSession()->set($this->returnUrlParam, $url);
    }
    ...
    public function loginRequired($checkAjax = true)
    {
        ...
        if ($this->loginUrl !== null) {
            ...
            if ($loginUrl[0] !== Leaps::$app->requestedRoute) {
                return Leaps::$app->getResponse()->redirect($this->loginUrl);
            }
        }
        throw new ForbiddenHttpException(Leaps::t('leaps', 'Login Required'));
    }
    ...
    protected function renewIdentityCookie()
    {
        ...
        $value = Leaps::$app->getRequest()->getCookies()->getValue($name);
        if ($value !== null) {
            $data = json_decode($value, true);
            if (is_array($data) && isset($data[2])) {
                ...
                Leaps::$app->getResponse()->getCookies()->add($cookie);
            }
        }
    }
    ...
    public function switchIdentity($identity, $duration = 0)
    {
        ...
        if ($identity) {
            ...
            Leaps::$app->getResponse()->getCookies()->remove(new Cookie($this->identityCookie));
        }
    }
     ...
     * [[\Leaps\Rbac\ManagerInterface::checkAccess()]] to obtain the up-to-date access result. Note that this
    ...
    {
        return Leaps::$app->getAuthManager();
    }
}
