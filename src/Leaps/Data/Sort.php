 ...
 *           passed to [[\Leaps\Db\Query::orderBy()]] to construct a DB query. This property is read-only.
 *          
...
class Sort extends Object
{
	...
	/**
	 *
	 * @var \Leaps\Web\UrlManager the URL manager used for creating sort URLs. If not set,
	 ...
	 *         This can be passed to [[\Leaps\Db\Query::orderBy()]] to construct a DB query.
	 */
	...
	public function createUrl($attribute, $absolute = false)
	{
		...
		$params [0] = $this->route === null ? Leaps::$app->controller->getRoute () : $this->route;
		$urlManager = $this->urlManager === null ? Leaps::$app->getUrlManager () : $this->urlManager;
		if ($absolute) {
