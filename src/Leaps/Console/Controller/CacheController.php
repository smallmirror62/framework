...
class CacheController extends Controller
{
    ...
    public function actionFlushSchema($db = 'db')
    {
        ...
        try {
            ...
            $schema->refresh();
            $this->stdout("Schema cache for service \"$db\", was flushed.\n\n", Console::FG_GREEN);
        } catch (\Exception $e) {
    ...
    {
        $this->stdout("No cache services were found in the system.\n", Console::FG_RED);
    }
    ...
    {
        $this->stdout("The following cache services were NOT found:\n\n", Console::FG_RED);
    ...
    {
        $this->stdout("The following cache services were processed:\n\n", Console::FG_YELLOW);
    ...
    {
        $this->stdout("The following cache services will be flushed:\n\n", Console::FG_YELLOW);

        foreach ($cachesNames as $name) {
            $this->stdout("\t* $name \n", Console::FG_GREEN);
        }

        return $this->confirm("\nFlush above cache services?");
    }
    ...
    {
        $caches = [];
        $services = Leaps::$app->getServices();
        $findAll = ($cachesNames == []);
        ...
        foreach ($services as $name => $service) {
            ...
            if ($service instanceof Cache) {
                $caches[$name] = get_class($service);
            } elseif (is_array($service) && isset($service['className']) && $this->isCacheClass($service['className'])) {
                $caches[$name] = $service['className'];
            } elseif (is_string($service) && $this->isCacheClass($service)) {
                $caches[$name] = $service;
            }
        }
