 ...
 *
 * AssetManager is configured as an application component in [[\Leaps\Web\Application]] by default.
 * You can access that instance via `Leaps::$app->assetManager`.
 *
...
class AssetManager extends Service
{
	...
	protected function loadBundle($name, $config = [], $publish = true)
	{
		...
		$bundle = Leaps::createObject ( $config );
		if ($publish) {
			$bundle->publish ( $this );
		}
		return $bundle;
	...
	public function getAssetUrl($bundle, $asset)
	{
		if (($actualAsset = $this->resolveAsset ( $bundle, $asset )) !== false) {
			if (strncmp ( $actualAsset, '@web/', 5 ) === 0) {
				...
				$basePath = Leaps::getAlias ( "@webroot" );
				$baseUrl = Leaps::getAlias ( "@web" );
			} else {
				$asset = Leaps::getAlias ( $actualAsset );
	...
	public function getConverter()
	{
		if ($this->_converter === null) {
			$this->_converter = Leaps::createObject ( AssetConverter::className () );
		} elseif (is_array ( $this->_converter ) || is_string ( $this->_converter )) {
			if (is_array ( $this->_converter ) && ! isset ( $this->_converter ['className'] )) {
				$this->_converter ['className'] = AssetConverter::className ();
			}
			$this->_converter = Leaps::createObject ( $this->_converter );
		}
	...
	protected function hash($path)
	{
		...
		return sprintf ( '%x', crc32 ( $path . Leaps::getVersion () ) );
	}
}
