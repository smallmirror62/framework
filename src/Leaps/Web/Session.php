 ...
 *
 * Session is a Web application component that can be accessed via `Leaps::$app->session`.
 *
...
class Session extends Service implements \IteratorAggregate, \ArrayAccess, \Countable
{
    ...
    public function open()
    {
        ...
        if ($this->getIsActive()) {
            Leaps::info('Session started', __METHOD__);
            $this->updateFlashCounters();
        } else {
            $error = error_get_last();
            $message = isset($error['message']) ? $error['message'] : 'Failed to start session.';
            Leaps::error($message, __METHOD__);
        }
    }
    ...
    protected function registerSessionHandler()
    {
        if ($this->handler !== null) {
            if (!is_object($this->handler)) {
                $this->handler = Leaps::createObject($this->handler);
            }
     ...
     * ```php
     * <?php
     * foreach (Leaps::$app->session->getAllFlashes() as $key => $message) {
