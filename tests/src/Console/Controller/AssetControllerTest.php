...
use Leaps\Console\controllers\AssetController;

/**
 * Unit test for [[\Leaps\Console\Controller\AssetController]].
 * @see AssetController
 *
 * @group console
 */
class AssetControllerTest extends TestCase
{
	...
	public function setUp()
	{
		...
		$this->testFilePath = Leaps::getAlias ( '@leapsunit/runtime' ) . DIRECTORY_SEPARATOR . str_replace ( '\\', '_', get_class ( $this ) ) . uniqid ();
	...
	protected function createCompressConfig(array $bundles)
	{
		...
		$classNumber ++;
		$className = $this->declareAssetBundleClass ( [
			'class'=> 'AssetBundleAll' . $classNumber
		] );
		$baseUrl = '/test';
		$config = [
			'bundles'=> $bundles,
...
EOL;
	...
	public function testActionTemplate()
	{
		...
		$this->runAssetControllerAction ( 'template', [
			$configFileName
		] );
	...
	public function testActionCompress()
	{
		...
		// When :
		$this->runAssetControllerAction ( 'compress', [
			$configFile,
			$bundleFile
		] );
	...
	public function testDetectCircularDependency()
	{
		...
		$this->setExpectedException ( 'Leaps\Console\Exception', $expectedExceptionMessage );
	...
	{
		$adjustedCssContent = $this->invokeAssetControllerMethod ( 'adjustCssUrl', [
			$cssContent,
			$inputFilePath,
			$outputFilePath
		] );
	...
	{
		$expectedRealPath = str_replace ( [
			'/',
			'\\'
		], DIRECTORY_SEPARATOR, $expectedRealPath );
		$realPath = $this->invokeAssetControllerMethod ( 'findRealPath', [
			$sourcePath
		] );
...
/**
 * Mock class for [[\Leaps\Console\Controller\AssetController]]
 */
