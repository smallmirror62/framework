...
/* @var $handler \Leaps\Web\ErrorHandler */
?>
...
<body>
    <div class="header">
        <?php if ($exception instanceof \Leaps\Base\ErrorException): ?>
        ...
        <?php if ($exception instanceof \Leaps\Db\Exception && !empty($exception->errorInfo)) {
        ...
        <ul>
            ...
            <?php for ($i = 0, $trace = $exception->getTrace(), $length = count($trace); $i < $length; ++$i): ?>
                <?= $handler->renderCallStackItem(@$trace[$i]['file'] ?: null, @$trace[$i]['line'] ?: null,
                    @$trace[$i]['className'] ?: null, @$trace[$i]['function'] ?: null, @$trace[$i]['args'] ?: [], $i + 2) ?>
            <?php endfor; ?>
        </ul>
    </div>
...
window.onload = function() {
    ...
    <?php if (method_exists($this, 'endBody')) $this->endBody(); // to allow injecting code into body (mostly by Leaps Debug Toolbar) ?>
</body>
