...
class Command extends Service
{
    ...
    /**
     * @var \Leaps\Cache\Dependency the dependency to be associated with the cached query result for this command
     * @see cache()
     */
     ...
     * @param \Leaps\Cache\Dependency $dependency the cache dependency associated with the cached query result.
     ...
     * @param string $type the column type. [[\Leaps\Db\QueryBuilder::getColumnType()]] will be called
    ...
    protected function queryInternal($method, $fetchMode = null)
    {
        ...
        Leaps::info($rawSql, 'Leaps\Db\Command::query');

        if ($method !== '') {
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheDependency);
            if (is_array($info)) {
                /* @var $cache \Leaps\Cache\Cache */
                $cache = $info[0];
                $cacheKey = [
                    __CLASS__,
                    $method,
                ...
                if (is_array($result) && isset($result[0])) {
                    Leaps::trace('Query result served from cache', 'Leaps\Db\Command::query');
                    return $result[0];
                }
            }
        }
        ...
        $token = $rawSql;
        try {
            Leaps::beginProfile($token, 'Leaps\Db\Command::query');
            ...
            Leaps::endProfile($token, 'Leaps\Db\Command::query');
        } catch (\Exception $e) {
            Leaps::endProfile($token, 'Leaps\Db\Command::query');
            throw $this->db->getSchema()->convertException($e, $rawSql);
        }
        ...
        if (isset($cache, $cacheKey, $info)) {
            ...
            Leaps::trace('Saved query result in cache', 'Leaps\Db\Command::query');
        }
