...
class BaseUrl
{
     ...
     *
     * This method will use [[\Leaps\Web\UrlManager]] to create a URL.
     *
     ...
     *
     * - If the route is an empty string, the current [[\Leaps\Web\Controller::route|route]] will be used;
     * - If the route contains no slashes at all (e.g. `index`), it is considered to be an action ID
     *   of the current controller and will be prepended with [[\Leaps\Web\Controller::uniqueId]];
     * - If the route has no leading slash (e.g. `site/index`), it is considered to be a route relative
     *   to the current module and will be prepended with the module's [[\Leaps\Base\Module::uniqueId|uniqueId]].
     *
    ...
    public static function toRoute($route, $scheme = false)
    {
        ...
        if ($scheme) {
            return Leaps::$app->getUrlManager()->createAbsoluteUrl($route, is_string($scheme) ? $scheme : null);
        } else {
            return Leaps::$app->getUrlManager()->createUrl($route);
        }
    }
    ...
    {
        $route = Leaps::getAlias((string) $route);
        ...
        if (strpos($route, '/') === false) {
            ...
            return $route === '' ? Leaps::$app->controller->getRoute() : Leaps::$app->controller->getUniqueId() . '/' . $route;
        } else {
            // relative to module
            return ltrim(Leaps::$app->controller->module->getUniqueId() . '/' . $route, '/');
        }
    }
     ...
     * [[\Leaps\Web\UrlManager::hostInfo]]) will be returned. If `$url` is already an absolute URL, its scheme
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        if ($url === '') {
            $url = Leaps::$app->getRequest()->getUrl();
        }
        ...
        if (($pos = strpos($url, ':')) == false || !ctype_alpha(substr($url, 0, $pos))) {
            ...
            $url = Leaps::$app->getUrlManager()->getHostInfo() . '/' . ltrim($url, '/');
        }
    ...
    {
        $url = Leaps::$app->getUrlManager()->getBaseUrl();
        if ($scheme) {
            $url = Leaps::$app->getUrlManager()->getHostInfo() . $url;
            if (is_string($scheme) && ($pos = strpos($url, '://')) !== false) {
                $url = $scheme . substr($url, $pos);
            }
        }
     ...
     * later by [[previous()]]. If not set, it will use [[\Leaps\Web\User::returnUrlParam]].
     * @see previous()
     */
    public static function remember($url = '', $name = null)
    {
        ...
        if ($name === null) {
            Leaps::$app->getUser()->setReturnUrl($url);
        } else {
            Leaps::$app->getSession()->set($name, $url);
        }
    }
     ...
     * If not set, it will use [[\Leaps\Web\User::returnUrlParam]].
    ...
    {
        if ($name === null) {
            return Leaps::$app->getUser()->getReturnUrl();
        } else {
            return Leaps::$app->getSession()->get($name);
        }
    }

    /**
     * Returns the canonical URL of the currently requested page.
     * The canonical URL is constructed using the current controller's [[\Leaps\Web\Controller::route]] and
     * [[\Leaps\Web\Controller::actionParams]]. You may use the following code in the layout view to add a link tag
     * about canonical URL:
     *
     * ```php
    ...
    {
        $params = Leaps::$app->controller->actionParams;
