...
use Leaps\Web\TooManyRequestsHttpException;
 ...
 *     return [
 *         'rateLimiter' => [
 *             'className' => \Leaps\Filter\RateLimiter::className(),
 *         ],
 *     ];
 * }
 * ```
...
class RateLimiter extends ActionFilter
{
     ...
     * If not set, it will take the value of `Leaps::$app->user->getIdentity(false)`.
     */
    public $user;
    /**
    ...
    {
        $user = $this->user ? : Leaps::$app->getUser()->getIdentity(false);
        if ($user instanceof RateLimitInterface) {
            Leaps::trace('Check rate limit', __METHOD__);
            $this->checkRateLimit(
                $user,
                $this->request ? : Leaps::$app->getRequest(),
                $this->response ? : Leaps::$app->getResponse(),
                $action
            );
        } elseif ($user) {
            Leaps::info('Rate limit skipped: "user" does not implement RateLimitInterface.', __METHOD__);
        } else {
            Leaps::info('Rate limit skipped: user not logged in.', __METHOD__);
        }
        return true;
    }
