 ...
 *
 * ```
 * leaps migrate --migrationPath=@Leaps/Rbac/migrations/
 * ```
 *
...
class DbManager extends BaseManager
{
	...
	public function getPermissionsByUser($userId)
	{
		...
		$query = (new Query ())->select ( 'item_name' )->from ( $this->assignmentTable )->where ( [ 
	...
	{
		if (! $this->supportsCascadeUpdate ()) {
			$names = (new Query ())->select ( [ 
				'name' 
			] )->from ( $this->itemTable )->where ( [ 
				'type' => $type 
			] )->column ( $this->db );
			if (empty ( $names )) {
				return;
