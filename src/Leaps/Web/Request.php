 ...
 *
 * Request is configured as an application component in [[\Leaps\Web\Application]] by default.
 * You can access that instance via `Leaps::$app->request`.
 *
...
class Request extends \Leaps\Base\Request
{
     ...
     * You may use [[\Leaps\Helper\Html::beginForm()]] to generate his hidden input.
     *
     * In JavaScript, you may get the values of [[csrfParam]] and [[csrfToken]] via `leaps.getCsrfParam()` and
     * `leaps.getCsrfToken()`, respectively. The [[\Leaps\Web\LeapsAsset]] asset must be registered.
     * You also need to include CSRF meta tags in your pages by using [[\Leaps\Helper\Html::csrfMetaTags()]].
     *
     ...
     * corresponding configurations for [[Leaps::createObject|creating the parser objects]].
     ...
     * ```
     * [
     *     'application/json' => 'Leaps\Web\JsonParser',
     * ]
     * ```
     *
    ...
    {
        $result = Leaps::$app->getUrlManager()->parseRequest($this);
        if ($result !== false) {
            ...
            throw new NotFoundHttpException(Leaps::t('leaps', 'Page not found.'));
        }
    }
     ...
     * @throws \Leaps\Base\InvalidConfigException if a registered parser does not implement the [[RequestParserInterface]].
     * @see getMethod()
     * @see getBodyParam()
     * @see setBodyParams()
     */
    ...
    protected function loadCookies()
    {
        ...
        if ($this->enableCookieValidation) {
            ...
            foreach ($_COOKIE as $name => $value) {
                ...
                $data = Leaps::$app->getSecurity()->validateData($value, $this->cookieValidationKey);
                if ($data === false) {
                    continue;
                }
    ...
    protected function loadCsrfToken()
    {
        if ($this->enableCsrfCookie) {
            ...
            return Leaps::$app->getSession()->get($this->csrfParam);
        }
    }
    ...
    {
        $token = Leaps::$app->getSecurity()->generateRandomString();
        if ($this->enableCsrfCookie) {
            $cookie = $this->createCsrfCookie($token);
            Leaps::$app->getResponse()->getCookies()->add($cookie);
        } else {
            Leaps::$app->getSession()->set($this->csrfParam, $token);
        }
        return $token;
    }
