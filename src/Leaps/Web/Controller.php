...
class Controller extends \Leaps\Base\Controller
{
     ...
     * This method is invoked by [[\Leaps\Base\Action]] when it begins to run with the given parameters.
     * This method will check the parameter names that the action requires and return
     * the provided parameters according to the requirement. If there is any missing parameter,
     * an exception will be thrown.
     * @param \Leaps\Base\Action $action the action to be bound with parameters
    ...
    public function bindActionParams($action, $params)
    {
        ...
        foreach ($method->getParameters() as $param) {
            ...
            if (array_key_exists($name, $params)) {
                if ($param->isArray()) {
                    ...
                    throw new BadRequestHttpException(Leaps::t('leaps', 'Invalid data received for parameter "{param}".', [
                        'param' => $name,
                    ]));
                }
                unset($params[$name]);
        ...
        if (!empty($missing)) {
            throw new BadRequestHttpException(Leaps::t('leaps', 'Missing required parameters: {params}', [
    ...
    public function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            if ($this->enableCsrfValidation && Leaps::$app->getErrorHandler()->exception === null && !Leaps::$app->getRequest()->validateCsrfToken()) {
                throw new BadRequestHttpException(Leaps::t('leaps', 'Unable to verify your data submission.'));
            }
            return true;
        }
