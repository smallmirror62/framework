 ...
 * @since 2.0
 */
class AssetConverter extends Service implements AssetConverterInterface
{
	/**
	 *
	 ...
	 * @throws \Leaps\Base\Exception when the command fails and LEAPS_DEBUG is true.
	 *         In production mode the error will be logged.
	 */
	protected function runCommand($command, $basePath, $asset, $result)
	{
		$command = Leaps::getAlias ( $command );
		...
		if ($status === 0) {
			Leaps::trace ( "Converted $asset into $result:\nSTDOUT:\n$stdout\nSTDERR:\n$stderr", __METHOD__ );
		} elseif (LEAPS_DEBUG) {
			throw new Exception ( "AssetConverter command '$command' failed with exit code $status:\nSTDOUT:\n$stdout\nSTDERR:\n$stderr" );
		} else {
			Leaps::error ( "AssetConverter command '$command' failed with exit code $status:\nSTDOUT:\n$stdout\nSTDERR:\n$stderr", __METHOD__ );
		}
