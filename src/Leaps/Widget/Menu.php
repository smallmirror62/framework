 ...
 *         ]],
 *         ['label' => 'Login', 'url' => ['site/login'], 'visible' => Leaps::$app->user->isGuest],
 *     ],
 * ]);
 * ~~~
 *
...
class Menu extends Widget
{
    ...
    public function run()
    {
        if ($this->route === null && Leaps::$app->controller !== null) {
            $this->route = Leaps::$app->controller->getRoute();
        }
        if ($this->params === null) {
            $this->params = Leaps::$app->request->getQueryParams();
        }
    ...
    protected function isItemActive($item)
    {
        if (isset($item['url']) && is_array($item['url']) && isset($item['url'][0])) {
            $route = Leaps::getAlias($item['url'][0]);
            if ($route[0] !== '/' && Leaps::$app->controller) {
                $route = Leaps::$app->controller->module->getUniqueId() . '/' . $route;
            }
