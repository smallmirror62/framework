...
trait ActiveRecordTestTrait
{
	...
	public function testFindScalar()
	{
		...
		/* @var $this TestCase|ActiveRecordTestTrait */
		// query scalar
		$customerName = $customerClass::find ()->where ( [
			'id'=> 2
		] )->scalar ( 'name' );
		...
		$customerId = $customerClass::find ()->where ( [
			'status'=> 2
		] )->scalar ( 'id' );
	...
	public function testFindCount()
	{
		...
		$this->assertEquals ( 1, $customerClass::find ()->where ( [
			'id'=> 1
		] )->count () );
		$this->assertEquals ( 2, $customerClass::find ()->where ( [
			'id'=> [
				1,
				2
			]
	...
	public function testFindComplexCondition()
	{
		...
		$this->assertEquals ( 2, count ( $customerClass::find ()->where ( [
			'OR',
			[
				'name'=> 'user1'
			],
		...
		$this->assertEquals ( 1, count ( $customerClass::find ()->where ( [
			'AND',
			[
				'name'=> [
					'user2',
	...
	public function testFindNullValues()
	{
		...
		$result = $customerClass::find ()->where ( [
			'name'=> null
		] )->all ();
	...
	public function testExists()
	{
		...
		/* @var $this TestCase|ActiveRecordTestTrait */
		$this->assertTrue ( $customerClass::find ()->where ( [
			'id'=> 2
		] )->exists () );
		$this->assertFalse ( $customerClass::find ()->where ( [
			'id'=> 5
		] )->exists () );
		$this->assertTrue ( $customerClass::find ()->where ( [
			'name'=> 'user1'
	...
	public function testFindLazy()
	{
		...
		$customer = $customerClass::findOne ( 2 );
		$this->assertFalse ( $customer->isRelationPopulated ( 'orders' ) );
		$orders = $customer->getOrders ()->where ( [
			'id'=> 3
		] )->all ();
		$this->assertFalse ( $customer->isRelationPopulated ( 'orders' ) );
	...
	public function testLink()
	{
		...
		$orderItem = $orderItemClass::findOne ( [
			'order_id'=> 1,
			'item_id'=> 3
		] );
	...
	public function testUnlinkAll()
	{
		...
		$this->assertEquals ( 3, $orderWithNullFKClass::find ()->count () );
		$this->assertEquals ( 2, $orderWithNullFKClass::find ()->where ( [
			'AND',
			[
				'id'=> [
					2,
	...
	public function testBooleanAttribute()
	{
		...
		$customers = $customerClass::find ()->where ( [
			'status'=> true
		] )->all ();
		$this->assertEquals ( 2, count ( $customers ) );
	...
	public function testFindEagerIndexBy()
	{
		...
		/* @var $order Order */
		$order = $orderClass::find ()->with ( 'itemsIndexed' )->where ( [
			'id'=> 1
		] )->one ();
